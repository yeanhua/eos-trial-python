...
from eos_trial import EosTrial, AsyncEosTrial, DefaultAioHttpClient
from eos_trial._utils import is_dict
...
logging.getLogger("eos_trial").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[EosTrial]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with EosTrial(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncEosTrial]:
